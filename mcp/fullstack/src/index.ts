...
const AnalyzeOutput = z.object({
  ...
  dataRequirements: z.array(
    z.object({
      entityName: z.string(),
      fields: z.array(z.object({ name: z.string(), type: z.string(), required: z.boolean().optional(), validation: z.string().optional() })),
  ...
  existingApiCalls: z.array(z.object({ endpoint: z.string(), method: z.string(), purpose: z.string().optional() })),
  formInputs: z.array(z.object({ name: z.string(), type: z.string(), validation: z.string().optional() })),
...
server.registerTool(
  ...
  async ({ componentPath, routePath, componentType }) => {
    const abs = path.isAbsolute(componentPath) ? componentPath : path.join(process.cwd(), componentPath)
    const content = await fs.readFile(abs, 'utf8')
    const apiCalls: { endpoint: string; method: string; purpose?: string }[] = []
    const endpointRegex = /(fetch\(['"]([^'"\n]+)['"]\)|axios\.(get|post|put|delete)\(['"]([^'"\n]+)['"]\))/g
    ...
    const dataRequirements = [
      { entityName: 'Unknown', fields, operations: ['create', 'read', 'update', 'delete'], relationships: [] }
    ]
    const output = { route: routePath, dataRequirements, existingApiCalls: apiCalls, formInputs, uiPatterns }
...
const GenOutput = z.object({
  ...
  artifacts: z.object({ prismaSchema: z.string().optional(), openApiSpec: z.any().optional(), generatedCode: z.array(z.object({ path: z.string(), content: z.string() })).optional() })
})
...
const EnhanceOutput = z.object({ success: z.boolean(), message: z.string(), modifiedPath: z.string().optional() })
...
server.registerTool(
  ...
  async ({ entity, operations, schema, relationships }) => {
    ...
    try {
      ...
      doc.paths[basePath] ||= {}
      if (operations.includes('read')) doc.paths[basePath].get = { tags: [entity], summary: `List ${entityLower}s`, responses: { '200': { description: `${entity} list` } } }
      if (operations.includes('create')) doc.paths[basePath].post = { tags: [entity], summary: `Create ${entityLower}`, responses: { '200': { description: `Created ${entityLower}` } } }
      const idPath = `${basePath}/{id}`
      doc.paths[idPath] ||= {}
      if (operations.includes('read')) doc.paths[idPath].get = { tags: [entity], summary: `Get ${entityLower} by ID`, responses: { '200': { description: entity } } }
      if (operations.includes('update')) doc.paths[idPath].put = { tags: [entity], summary: `Update ${entityLower}`, responses: { '200': { description: `Updated ${entityLower}` } } }
      if (operations.includes('delete')) doc.paths[idPath].delete = { tags: [entity], summary: `Delete ${entityLower}`, responses: { '200': { description: 'Deleted' } } }
...
const DbInput = z.object({
  models: z.array(z.object({ name: z.string(), fields: z.array(z.object({ name: z.string(), type: z.string(), optional: z.boolean().optional() })) })),
  relationships: z.array(z.object({ from: z.string(), to: z.string(), type: z.enum(['one-to-many', 'many-to-many', 'one-to-one']) })),
  indexes: z.array(z.object({ model: z.string(), fields: z.array(z.string()) })).optional(),
  constraints: z.array(z.object({ model: z.string(), field: z.string(), type: z.string() })).optional()
})

const DbOutput = z.object({ success: z.boolean(), message: z.string(), modifiedPath: z.string().optional() })
...
server.registerTool(
  ...
  async ({ models }) => {
    ...
    try {
      ...
      const additions = models.map(m => buildPrismaModel(m.name, m.fields.map(f => ({ name: f.name, type: f.type })))).join('\n\n')
      const updated = `${raw}\n\n${additions}`
...
const ReactQueryOutput = z.object({ success: z.boolean(), message: z.string(), codeSample: z.string().optional() })
...
const ValidateInput = z.object({ route: z.string(), checkDatabase: z.boolean().optional(), checkOpenApi: z.boolean().optional(), checkHooks: z.boolean().optional() })

const ValidateOutput = z.object({ success: z.boolean(), report: z.object({ warnings: z.array(z.string()), errors: z.array(z.string()), checks: z.array(z.string()) }) })
  ...
  async ({ route, checkDatabase = true, checkOpenApi = true, checkHooks = true }) => {
    ...
    if (checkDatabase) {
      ...
      try {
        ...
        if (!/model\s+User/.test(raw) || !/model\s+Post/.test(raw)) warnings.push('Core models missing')
      } catch {
...
const RulesOutput = z.object({ success: z.boolean(), rules: z.object({ validation: z.array(z.string()), business: z.array(z.string()), relationships: z.array(z.string()), api: z.array(z.string()) }) })
...
server.registerTool(
  ...
  async ({ rulesPath }) => {
    ...
    try {
      ...
      const output = { success: false, rules: { validation: [], business: [], relationships: [], api: [] } }
      return { content: [{ type: 'text', text: JSON.stringify(output) }], structuredContent: output }
    }
  }
)
...
function inferFields(text: string): { name: string; type: string }[] {
  ...
  for (const f of defaults) if (new RegExp(`\\b${f}\\b`, 'i').test(text)) fields.push({ name: f, type: f === 'status' ? 'String' : 'String' })
...
function buildReactQuerySample(lower: string, operations: ('create'|'read'|'update'|'delete')[], ui: 'list'|'form'|'detail') {
  const lines: string[] = []
  if (ui === 'list' && operations.includes('read')) {
    ...
    lines.push(`  const { data, isLoading, error } = useQuery({ queryKey: ['${lower}s'], queryFn: () => getApi${capitalize(lower)}s() })`)
  ...
  if (ui === 'detail' && operations.includes('read')) {
    ...
    lines.push(`  const { data } = useQuery({ queryKey: ['${lower}', id], queryFn: () => getApi${capitalize(lower)}sId(id) })`)
    lines.push(`  return null`)
    lines.push(`}`)
  }
  return lines.join('\n')
