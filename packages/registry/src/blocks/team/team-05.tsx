  ...
  return (
    <View className="w-full max-w-sm items-center gap-4 p-6">
        ...
        <View className="flex-row">
             {[1, 2, 3, 4].map((i) => (
                <Avatar key={i} className="border-2 border-background -ml-3 first:ml-0">
                    <AvatarImage source={{ uri: `https://i.pravatar.cc/150?u=${i}` }} />
                    <AvatarFallback><Text>U{i}</Text></AvatarFallback>
                </Avatar>
             ))}
             <View className="h-10 w-10 rounded-full bg-muted items-center justify-center border-2 border-background -ml-3">
