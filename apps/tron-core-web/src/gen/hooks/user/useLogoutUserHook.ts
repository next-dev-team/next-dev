...
export async function logoutUserHook(config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<LogoutUserQueryResponse, ResponseErrorConfig<Error>, unknown>({ method : "GET", url : `/user/logout`, baseURL : "https://petstore3.swagger.io/api/v3", ... requestConfig })  
  return res.data
}
 ...
 * {@link /user/logout}
 */
export function useLogoutUserHook<TData = LogoutUserQueryResponse, TQueryData = LogoutUserQueryResponse, TQueryKey extends QueryKey = LogoutUserQueryKey>(options: 
{
