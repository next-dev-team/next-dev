...
export async function addPetHook(data: AddPetMutationRequest, config: Partial<RequestConfig<AddPetMutationRequest>> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<AddPetMutationResponse, ResponseErrorConfig<AddPet405>, AddPetMutationRequest>({ method : "POST", url : `/pet`, baseURL : "https://petstore3.swagger.io/api/v3", data : requestData, ... requestConfig })  
  return res.data
}
...
export function useAddPetHook<TContext>(options: 
{
 ...
 = {}) {
  ...
  const baseOptions = addPetMutationOptionsHook(config) as UseMutationOptions<AddPetMutationResponse, ResponseErrorConfig<AddPet405>, {data: AddPetMutationRequest}, TContext>

  return useMutation<AddPetMutationResponse, ResponseErrorConfig<AddPet405>, {data: AddPetMutationRequest}, TContext>({
    ...baseOptions,
    mutationKey,
    ...mutationOptions,
  }, queryClient) as UseMutationResult<AddPetMutationResponse, ResponseErrorConfig<AddPet405>, {data: AddPetMutationRequest}, TContext>
}