---
title: Shop Quick View
description: Product quick view modal content.
---

import { BlockPreviewCard } from '@docs/components/preview-card';
import { ShopUi04 } from '@docs/components/blocks';
import { CommandTabs } from '@docs/components/command-tabs';
import { Step, Steps } from 'fumadocs-ui/components/steps';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@docs/components/ui/tabs';

<BlockPreviewCard
  preview={
    <div className="w-full">
      <ShopUi04 />
    </div>
  }
/>

## Installation

<Tabs defaultValue="cli">
  <TabsList>
    <TabsTrigger id="cli" value="cli">CLI</TabsTrigger>
    <TabsTrigger id="manual" value="manual">Manual</TabsTrigger>
  </TabsList>
  <TabsContent value="cli">
    <CommandTabs args={["add", "shop-ui-04"]} />
  </TabsContent>
  <TabsContent value="manual">
    <Steps>
      <Step>
        **Copy and paste the following code into your project.**

        ```json doc-gen:file
        {
          "file": "./node_modules/@rnr/registry/src/blocks/shop-ui/shop-ui-04.tsx",
          "codeblock": {
            "lang": "tsx"
          }
        }
        ```
      </Step>
    </Steps>

  </TabsContent>
</Tabs>
