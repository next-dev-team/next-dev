...
- The example components are found in the `packages/rn-ui` folder.
...
<Callout>
  **Note:** The Reusables UI is installed in `packages/rn-ui/reusables`.
  
  If you want to use more `react-native-reusables` components as dependencies, you have to install them
  through the [CLI](https://reactnativereusables.com/docs/cli) inside `packages/rn-ui`.
</Callout>

## Building your own library

You can start by changing the `packages/rn-ui` folder name and replace all the references to it through the codebase.
...
- `packages/rn-ui/package.json` to update the package name
- `packages/rn-ui/src/components/ui/*` to create your components
- `packages/rn-ui/src/examples/*` to update the example components
- `packages/rn-ui/src/blocks/*` to update the examples blocks
