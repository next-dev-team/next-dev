...
- The example components are found in the `packages/rn-nativewind` folder.
...
<Callout>
  **Note:** The Reusables UI is installed in `packages/rn-nativewind/reusables`.
  
  If you want to use more `react-native-reusables` components as dependencies, you have to install them
  through the [CLI](https://reactnativereusables.com/docs/cli) inside `packages/rn-nativewind`.
</Callout>

## Building your own library

You can start by changing the `packages/rn-nativewind` folder name and replace all the references to it through the codebase.

<Callout>
  - All `package.json` files start with `@rnr/...` and you will need to update them with your own library name too.
  - All the imports throughout the codebase use aliases defined in `tsconfig.json` files. You can search by `@/rn-ui` and replace it with the new name of the folder.
</Callout>
...
- `packages/rn-nativewind/package.json` to update the package name
- `packages/rn-nativewind/src/components/ui/*` to create your components
- `packages/rn-nativewind/src/examples/*` to update the example components
- `packages/rn-nativewind/src/blocks/*` to update the examples blocks
