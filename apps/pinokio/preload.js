...
window.electronAPI = {
  ...
  captureScreenshot: (screenshotRequest) => {
    return ipcRenderer.invoke("pinokio:capture-screenshot-debug", {
      screenshotRequest,
    });
  },
...
(function initInspector() {
  ...
  const findIframeCandidates = () => {
    const list = Array.from(document.querySelectorAll("iframe")).map(
      (iframe, index) => {
  ...
  const selectVisibleIframe = () => {
    ...
    candidates.slice(0, 3).forEach((candidate) => {
      ...
      log(
        `candidate[${candidate.index}] src=${
          candidate.src || "<empty>"
        } visible=${candidate.visible ? "yes" : "no"} size=${Math.round(
          rect.width
    ...
    log(
      `selected iframe src=${chosen.src || "<empty>"} visible=${
        chosen.visible ? "yes" : "no"
      } size=${Math.round(chosen.rect?.width || 0)}x${Math.round(
        chosen.rect?.height || 0
      )}`
    );
    const siblingsWithSameUrl = candidates.filter(
      (candidate) => candidate.src === chosen.src
    );
  ...
  const ensureOverlay = () => {
    if (
      ...
      document.body.contains(state.overlay.container)
    ) {
    ...
    const orphaned = Array.from(
      document.querySelectorAll(".pinokio-inspector-overlay")
    );
  ...
  const startInspector = async (button) => {
    ...
    showOverlay(
      "Inspect mode enabled – hover items and click to capture.",
      target.url || "",
      null
    );
  ...
  const handleToggleClick = (event) => {
    ...
    const isTrigger =
      ...
      (button.dataset &&
        button.dataset.tippyContent === "Switch to inspect mode");
    if (!isTrigger) {
      return;
    }
  ...
  const handleInspectorMessage = (event) => {
    ...
    const frameUrl =
      typeof data.frameUrl === "string" ? data.frameUrl : state.lastUrl;
    ...
    if (data.type === "update") {
      const label = data.nodeName
        ? `<${String(data.nodeName).toLowerCase()}>`
        : "";
    ...
    if (data.type === "complete") {
      ...
      showOverlay(
        "Element captured. Inspect again or close.",
        frameUrl || "",
        html,
        screenshot
      );
  ...
  ipcRenderer.on("pinokio:inspector-error", (_event, payload) => {
    const message =
      payload && payload.message ? payload.message : "Inspect mode ended.";
    hideOverlay();
    showOverlay(
      message,
      payload && payload.frameUrl ? payload.frameUrl : "",
      null
    );
    resetState();
  });
  ...
  const processScreenshotRequest = async (
    screenshotRequest,
    messageId,
    source
  ) => {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (
      typeof event.data.frameName === "string" &&
      event.data.frameName.trim()
    ) {
      payload.frameName = event.data.frameName.trim();
    }
    if (
      typeof event.data.frameNodeId === "string" &&
      event.data.frameNodeId.trim()
    ) {
    ...
    window.electronAPI
      ...
      .then(() => {
        try {
          console.log(
            "[Inspector] ipc-start-success " + JSON.stringify(payload)
          );
        } catch (_) {}
      })
      .catch((error) => {
        ...
        try {
          console.log(
            "[Inspector] ipc-start-error " + JSON.stringify({ message })
          );
        } catch (_) {}
        resetState();
      });
