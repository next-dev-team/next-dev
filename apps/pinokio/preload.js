...
window.electronAPI = {
  ...
  pterm: (args) => ipcRenderer.invoke('run-pterm', args),
};
(function initInspector() {
  ...
  const selectVisibleIframe = () => {
    ...
    log(
      ...
      } size=${Math.round(chosen.rect?.width || 0)}x${Math.round(chosen.rect?.height || 0)}`,
    );
  ...
  const handleInspectorMessage = (event) => {
    ...
    if (data.type === 'update') {
      ...
      showOverlay(
        label ? `Hovering ${label}` : 'Inspect mode enabled – hover items and click to capture.',
        frameUrl || '',
        null,
      );
      return;
